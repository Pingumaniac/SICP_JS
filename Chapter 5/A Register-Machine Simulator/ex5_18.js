...
of the machine. You are to implement a procedure

set_breakpoint(⟨machine⟩, ⟨label⟩, ⟨n⟩)

that sets a breakpoint just before the nth instruction aer
the given label. For example,

set-breakpoint(gcd-machine, "test-b", 4)

installs a breakpoint in gcd-machine just before the assignment
to register a. When the simulator reaches the breakpoint
it should print the label and the offset of the breakpoint
and stop executing instructions. Alyssa can then use
get_register_contents and set_register_contents to
