function compile(component, target, linkage) {
    ...
    } else if (is_lambda_expression(component)) { // modified for ex 5.43
    ...
    } else if (is_block(component)) { // modified for ex 5.43
...
// modified for ex 5.43
function compile_block(exp, target, linkage, compile_time_env) {
    const body = block_body(exp);
    const locals = scan_out_declarations(body);
    const unassigneds = list_of_unassigneds(locals);
    const new_compile_time_env = extendEnvironment(locals, compile_time_env);
    return append_instruction_sequences(
        make_instruction_sequence(["env"], ["env"],
            [
                assign("env", [op("extend_environment"), constant(locals), constant(unassigneds), reg("env")])
            ]),
...
// modified compile_lambda functions for ex 5.43
function compile_lambda_expression(component, target, linkage, compile_time_env) {
    ...
    return append_instruction_sequences(
        tack_on_instruction_sequence(
            ...
            compile_lambda_body(component, fun_entry, "next", compile_time_env)),
        after_lambda);
}
function compile_lambda_body(exp, target, linkage, compile_time_env) {
    const params = lambda_parameter_symbols(exp);
    const new_compile_time_env = extendEnvironment(params, compile_time_env);
    return append_instruction_sequences(
        make_instruction_sequence(["env", "fun", "argl"], ["env"],
            [assign("env", [op("compiled_function_env"), reg("fun")]),
            assign("env", [op("extend_environment"), constant(params), reg("argl"), reg("env")])]),
        compile(append_return_undefined(lambda_body(exp)), target, "next", new_compile_time_env)
    );
}
