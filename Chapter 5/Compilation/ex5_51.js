...
// ex 5.51
let global_environment = initial_global_environment();
function compile_and_run(program) {
    ...
    global_environment = extend_environment(toplevel_names, unassigneds, global_environment);
    set_current_environment(global_environment);
...
function EC_evaluate(input) {
    if (input.startsWith('function')) {
        return compile_and_run(parse(input));
    } else {
        return evaluate_expression(input, global_environment);
    }
}
// end of ex 5.51
