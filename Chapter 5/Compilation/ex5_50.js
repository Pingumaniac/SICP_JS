...
function compile_name(component, target, linkage, compile_time_env) {
    ...
    if (lexical_address.type === 'constant') {
        return end_with_linkage(linkage, make_instruction_sequence([], [target],
            [assign(target, [constant(lexical_address.value)])]));
    }
...
function symbol_of_name(component) { return head(tail(component)); }
function symbol_of_function_call(component) { return head(tail(component)); }
function compile_expression(expr) {
    if (expr.type === 'binary_operation') {
        const left = compile_expression(expr.left);
        const right = compile_expression(expr.right);
        ...
        const symbol_info = find_symbol(expr.name, compile_time_env);
        if (symbol_info.type === 'constant') {
            return symbol_info.value;
        }
        return `lookup_variable_value(${expr.name}, env)`;
    }
}

// ex 5.50
function compile_function_call(target, linkage, compile_time_env) {
    const function_symbol = symbol_of_function_call(component);
    const function_info = find_symbol(function_symbol, compile_time_env);
    const after_call = make_label("after_call");
    const compiled_linkage = linkage === "next" ? after_call : linkage;

    if (function_info.type === 'primitive') {
        return end_with_linkage(linkage, make_instruction_sequence(["fun", "argl"], [target],
            [assign(target, [op("apply_primitive_function"), reg("fun"), reg("argl")])]));
    } else if (function_info.type === 'compiled') {
        return compile_fun_appl(target, linkage);
    } else if (function_info.type === 'interpreted') {
        const compound_return = make_label("compound_return");
        if (target === "val" && linkage !== "return") {
            return make_instruction_sequence(["fun"], all_regs,
                [assign("continue", label(linkage)), save("continue"),
                    assign("fun", reg("compapp")), go_to(reg("fun"))]);
        } else if (target !== "val" && linkage !== "return") {
            return make_instruction_sequence(["fun"], all_regs,
                [assign("continue", label(compound_return)), save("continue"),
                    assign("fun", reg("compapp")), go_to(reg("fun")),
                    compound_return, assign(target, reg("val")), go_to(label(linkage))]);
        } else if (target === "val" && linkage === "return") {
            return make_instruction_sequence(["fun", "continue"], all_regs,
                [save("continue"), assign("fun", reg("compapp")), go_to(reg("fun"))]);
        } else {
            throw new Error("return linkage, target not val -- compile");
        }
    } else {
        throw new Error("Unknown function type -- compile");
    }
}
function compile_fun_appl(target, linkage) {
    const fun_return = make_label("fun_return");
    const compound_return = make_label("compound_return");
    const common_sequence = make_instruction_sequence(["fun"], all_regs,
