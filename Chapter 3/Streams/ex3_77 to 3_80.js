...
const make_zero_crossings = (input_stream, last_value) => {
    return [sign_change_detector(input_stream, last_value),
        () => make_zero_crossings(smooth(input_stream), stream_ref(input_stream, 0))];
}
...
function integral(delayed_integrand, initial_value, dt) {
    ...
    if (integrand === null) {
        ...
        return [initial_value, () => integral(delayed_integrand, initial_value + dt * integrand[0], dt)];
    }
}

// ex 3.78
const solve_2nd = (a, b, dt, y0, dy0) => {
    const y = integral(() => dy, y0, dt);
    const dy = integral(() => ddy, dy0, dt);
    const ddy = stream_map_2((y, dy) => a * y + b * dy, y, dy);
    return y;
}

// ex 3.79
const solve_2nd_generalized = (f, dt, y0, dy0) => {
    const y = integral(() => dy, y0, dt);
    const dy = integral(() => ddy, dy0, dt);
    const ddy = stream_map_2((y, dy) => f(y, dy), y, dy);
    return y;
}

// ex 3.80
const RLC = (R, L, C, dt) => {
    const vC = integral(() => dvC, 0, dt);
    const iL = integral(() => diL, 0, dt);
