...
const integral = (integrand, initial_value, dt) => {
    const integ = [initial_value, () => add_streams(scale_stream(integrand, dt), integ)];
    return integ;
}

// ex 3.73
const RC = (R, C, dt) => {
    const RC = [0, () => add_streams(scale_stream(RC, -1/(R*C)), stream_map(x => 1, RC))];
    return RC;
}
...
/* ex 3.74
const make_zero_crossings = (input_stream, last_value) => {
    return [sign_change_detector(input_stream, last_value),
        () => make_zero_crossings(input_stream, stream_ref(input_stream, 0))];
}
const zero_crossings_original = make_zero_crossings(sense_data, 0);
const zero_crossings = stream_map_2(sign_change_detector, sense_data, [stream_ref(sense_data, 0), () => sense_data]);
*/

// ex 3.75
const make_zero_crossings = (input_stream, last_value, last_avpt) => {
    let avpt = (stream_ref(input_stream, 0) + last_value) / 2;
    return [sign_change_detector(avpt, last_avpt),
        () => make_zero_crossings(stream_tail(input_stream), input_stream[0], avpt)];
}

// ex 3.76
const smooth = input_stream => stream_map_2((x, y) => (x + y) / 2, input_stream, stream_tail(input_stream));
const make_zero_crossings_2 = (input_stream, last_value) => {
    return [sign_change_detector(input_stream, last_value),
        () => make_zero_crossings_2(smooth(input_stream), stream_ref(input_stream, 0))];
}
