...
/* Exercise 2.64
(a) Short paragraph of how partial tree works:

The function partial_tree takes in a list of elements and the number of elements in the list.
It then recursively calls itself to create a left tree and a right tree.
The left tree is created by calling partial_tree on the 1st half of the list.
The right tree is created by calling partial_tree on the 2nd half of the list.
The function then returns a tree with the 1st element of the remaining list as the entry,
the left tree as the left branch, and the right tree as the right branch.
The function then returns the tree and the remaining list.

Draw a tree produced by list_to_tree for the list list(1, 3, 5, 7, 9, 11):
(1 (3 (5 (7 (9 (11 () ()) ()) ()) ()) ())

(b)
The order of growth of the number of steps required by partial_tree to construct a tree of size n is O(n).
THhs is because the function calls itself twice, once for the left tree and once for the right tree.
Each call to partial_tree takes O(n) steps because it calls itself recursively on a list of size n/2.

Conclusion: the order of growth =  O(n).
*/
