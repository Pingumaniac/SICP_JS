...
function is_element_of_set(x, set) { return is_null(set) ? false : x === head(set) ? true : is_element_of_set(x, tail(set)); }
function adjoin_set(x, set) { return is_element_of_set(x, set) ? set : pair(x, set); }
function intersection_set(set1, set2) {
    return is_null(set1) ? null :
    is_element_of_set(head(set1), set2) ? pair(head(set1), intersection_set(tail(set1), set2)) :
    intersection_set(tail(set1), set2);
}
function union_set(set1, set2) {
    return is_null(set1) ? set2 :
    is_element_of_set(head(set1), set2) ? union_set(tail(set1), set2) :
    pair(head(set1), union_set(tail(set1), set2));
}

// now allow duplicates
function is_element_of_set_with_duplicates(x, set) {
    // assume that set can have duplicates
    return is_null(set) ? false : x === head(set) ? true : is_element_of_set_with_duplicates(x, tail(set));
}
function adjoin_set_with_duplicates(x, set) { return pair(x, set); }
function intersection_set_with_duplicates(set1, set2) {
    return is_null(set1) ? null :
    is_element_of_set_with_duplicates(head(set1), set2) ? pair(head(set1), intersection_set_with_duplicates(tail(set1), set2)) :
    intersection_set_with_duplicates(tail(set1), set2);
}
function union_set_with_duplicates(set1, set2) {
    return is_null(set1) ? set2 :
    is_element_of_set_with_duplicates(head(set1), set2) ? union_set_with_duplicates(tail(set1), set2) :
    pair(head(set1), union_set_with_duplicates(tail(set1), set2));
}

/*
Efficiency comparison:

1.  is_element_of_set vs is_element_of_set_with_duplicates
    is_element_of_set_with_duplicates is more efficient because it does not need to check for duplicates

2. adjoin_set vs adjoin_set_with_duplicates
    adjoin_set_with_duplicates is less efficient because it needs adjoin duplicate elements

3. intersection_set vs intersection_set_with_duplicates
    intersection_set is more efficient because it does not need to check for duplicates

4. union_set vs union_set_with_duplicates
   union_set is more efficient because it does not need to check for duplicates

Applications for duplicative representation of unordered list over non-duplicate unordered list as sets?
1. When we want to keep track of the number of times an element appears in a set
2. When we want to keep track of the order of elements in a set
3. When we want to keep track of the number of times an element appears in a set and the order of elements in a set
*/
