...
const apply_generic = (op, args) => {
    ...
    let fun = get(op, type_tags);
    if (fun) {
        return fun(map(contents, args));
    }
    let type1 = head(type_tags);
    let type2 = head(tail(type_tags));
    let a1 = head(args);
    let a2 = head(tail(args));
    let t1_to_t2 = get_coercion(type1, type2);
    let t2_to_t1 = get_coercion(type2, type1);
    let higher = raise(type1);
    let lower = raise(type2);
    if (higher && lower) {
        return apply_generic(op, list(higher(a1), a2));
    }
    if (t1_to_t2) {
    ...
    return "No method for these types";
}
