...
const remainder_terms = (L1, L2) => {
    ...
    if (L1 === null) {
        ...
        const new_c = div(coeff(t1), coeff(t2));
        const new_o = sub(order(t1), order(t2));
        const rest_of_result = remainder_terms(rest_terms(L1), L2);
        return head(tail(rest_of_result)); // 2nd term of rest of result
    }
}

const greatest_common_divisor = (p1, p2) => {
    const t1 = first_term(coeff(p1));
    const t2 = first_term(coeff(p2));
    const t3 = make_term(sub(order(t1), order(t2)), gcd_terms(coeff(t1), coeff(t2)));
    return make_polynomial(variable(p1), gcd_terms(div_terms(coeff(p1), t3), div_terms(coeff(p2), t3)));
}

// ex 2.96
const pseudo_remainder_terms = (L1, L2) => {
    if (L1 === null) {
        return list(null, null);
    } else {
        const t1 = first_term(L1);
        ...
        if (order(t2) > order(t1)) {
            ...
            const new_c = div(coeff(t1), coeff(t2));
            const new_o = sub(order(t1), order(t2));
            const rest_of_result = pseudo_remainder_terms(rest_terms(L1), L2);
            return list(adjoin_term(make_term(new_o, new_c), first_term(rest_of_result)), second_term(rest_of_result));
        }
    }
}
...
const gcd_terms = (a, b) => {
    if (b === null) {
        let coeff_list = a.map(x => x[1]);
        let coeff_gcd = coeff_list.reduce((a, b) => gcd(a, b));
        return div_terms(a, make_term(0, coeff_gcd));
    } else {
        gcd_terms(b, pseudo_remainder_terms(a, b));
    }
}

const reduce_terms = (L) => {
    if (L === null) {
        return null;
    } else {
        const t = first_term(L);
        return adjoin_term(make_term(order(t), reduce(coeff(t))), reduce_terms(rest_terms(L)));
    }
}
...
const reduce = (x) => {
    ...
    const d = denom(x);
    const g = gcd(n, d);
    return make_rational(div(n, g), div(d, g));
}

const reduce_poly = (L) => {
    const t = first_term(L);
    return adjoin_term(make_term(order(t), reduce_terms(coeff(t))), reduce_poly(rest_terms(L)));
}

const reduce_integers = (n, d) => {
    const g = gcd(n, d);
    return list(div(n, g), div(d, g));
}
