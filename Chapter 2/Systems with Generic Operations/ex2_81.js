...
function apply_generic(op, args) {
    ...
    } else if (type_tags.length == 2 && type_tags[0] == type_tags[1]) {
        throw error(list(op, type_tags), "no method for these types");
    } else {
        let types = get_coercion(type_tags[0], type_tags[1]);
        if(types) {
            return apply_generic(op, [put_coercion(args[0], types[0]), put_coercion(args[1], types[1])]);
        } else {
            throw error(list(op, type_tags), "no method for these types");
        }
    }
...
/*
Answer for (a): If we call exp with two complex numbers as arguments,
it will throw an error because there is no method for these types.

Answer for (b): Yeap. Louis is correct that sth had to be done about coercion with arguments of same types.
This is because it does not check whether the operations be done without coercing to javascript number.
*/
