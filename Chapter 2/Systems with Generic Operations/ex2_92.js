...
const add_poly = (p1, p2) => {
    return is_same_variable(term(p1), term(p2)) ? make_polynomial(term(p1), add_terms(coefficients(p1), coefficients(p2))) : error(list(p1, p2), "Polys not in same var -- add_poly");
}
const mul_poly = (p1, p2) => {
    return is_same_variable(term(p1), term(p2)) ? make_polynomial(term(p1), mul_terms(coefficients(p1), coefficients(p2))) : error(list(p1, p2), "Polys not in same var -- mul_poly");
}
...
const install_polynomial_package = () => {
    ...
    const add_poly = (p1, p2) => {
        ...
        return is_same_variable(term(p1), term(p2)) ? make_polynomial(ordered_variable, add_terms(coefficients(p1), coefficients(p2))) : error(list(p1, p2), "Polys not in same var -- add_poly");
    }
    const sub_poly = (p1, p2) => {
        // impose ordering
        const ordered_variable = term(p1) < term(p2) ? term(p1) : term(p2);
        return is_same_variable(term(p1), term(p2)) ? make_polynomial(ordered_variable, sub_terms(coefficients(p1), coefficients(p2))) : error(list(p1, p2), "Polys not in same var -- sub_poly");
    }
    const mul_poly = (p1, p2) => {
        if (is_same_variable(term(p1), term(p2))) {
            return make_polynomial(term(p1), mul_terms(coefficients(p1), coefficients(p2)));
        } else {
            // impose ordering
            const ordered_variable = term(p1) < term(p2) ? term(p1) : term(p2);
            return make_polynomial(ordered_variable, mul_terms(coefficients(p1), coefficients(p2)));
        }
    }
    const div_poly = (p1, p2) => {
        if (is_empty_termlist(coefficients(p2))) {
            ...
            // impose ordering
            const ordered_variable = term(p1) < term(p2) ? term(p1) : term(p2);
            return make_polynomial(ordered_variable, div_terms(coefficients(p1), coefficients(p2)));
        }
    }
