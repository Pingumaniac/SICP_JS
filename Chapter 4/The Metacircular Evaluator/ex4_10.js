...
function extend_environment(symbols, vals, base_env) {
    ...
    let frame = {};
    for(let i = 0; i < symbols.length; i++) { frame[symbols[i]] = vals[i]; }
...
function lookup_symbol_value(symbol, env) {
    return environment_traversal(symbol, env, (frame, symbol) => frame[symbol]);
}
function assign_symbol_value(symbol, val, env) {
    environment_traversal(symbol, env, (frame, symbol) => { frame[symbol] = val; });
}
