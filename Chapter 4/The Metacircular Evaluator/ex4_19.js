...
function eval_declaration(component, env) {
    add_binding_to_frame(declaration_symbol(component),
    evaluate(declaration_value_expression(component), env), first_frame(env));
    return undefined;
}
function add_binding_to_frame(symbol, value, frame) {
    frame[0] = [symbol].concat(frame[0]);
    frame[1] = [value].concat(frame[1]);
}
/*
ex 4.19
...
I support Alyssia's view that the function should produce an error.

I cannot devise a way to implement internal declarations so that they behave as Eva prefers.
*/
