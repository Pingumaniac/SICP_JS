...
function add_lists(list1, list2) {
    return list1 === null ? list2 : list2 === null ? list1 :
    pair(head(list1) + head(list2), add_lists(tail(list1), tail(list2)));
}
function make_lazy_pair(x, y_thunk) { return ["lazy_pair", x, y_thunk]; }
function list(...items) {
    function list_helper(i) {
        if (i >= items.length) return null;
        return make_lazy_pair(items[i], () => helper(i + 1));
    }
    return list_helper(0);
}
function is_quoted(exp) { return is_tagged_list(exp, "quote"); }
function quoted_text(exp) { return head(tail(exp)); }
// ex 4.30
function is_lazy_pair(pair) { return is_tagged_list(pair, "lazy_pair"); }
function print_lazy_list(list, depth) {
    if (list === null) {
        return "null";
    } else if (depth === 0) {
        return "...";
    }  else if (!is_lazy_pair(list)) {
        throw new Error("Not a lazy pair");
    } else {
        const head_val = head(list);
        const tail_val = tail(list);
        return "(" + head_val + ", " + print_lazy_list(tail_val, depth - 1) + ")";
    }
}
...
function analyze_quoted_expression(exp) {
    if (Array.isArray(exp)) {
        return list(...exp.map(analyze_quoted_expression));
    } else {
        return exp;
    }
}
...
// Helper functions for while loop
...
function eval_conditional(component, env) {
    // evaluate -> actual_value for lazy evaluation
    if (is_truthy(actual_value(conditional_predicate(component, env)))) {
        return evaluate(conditional_consequent(component, env));
    } else {
        return evaluate(conditional_alternative(component, env));
    }
}
function list_of_unassigned(symbols) { return map(symbol => "*unassigned*", symbols); }
...
function make_function(parameters, body, env) {
    ...
    const unassigneds = list_of_unassigned(body_declarations);
...
// new functions for lazy evaluation
function list_of_arg_values(exps, env) { return map(exp => actual_value(exp, env), exps); }
function list_of_delayed_args(exps, env) { return map(exp => delay_it(exp, env), exps); }
// Representing thunks
function delay_it(exp, env) { return ["thunk", exp, env]; }
function is_thunk(obj) { return is_tagged_list(obj, "thunk"); }
function thunk_exp(obj) { return obj[1][0]; }
function thunk_env(obj) { return obj[1][1]; }
function is_evaluated_thunk(obj) { return is_tagged_list(obj, "evaluated_thunk"); }
function thunk_value(evaluated_thunk) { return evaluated_thunk[1][0]; }
function force_it(obj) {
    if (is_thunk(obj) && !obj.is_forced()) {
        const result = actual_value(thunk_exp(obj), thunk_env(obj));
        obj[0] = "evaluated_thunk";
        obj[1][0] = result;
        obj[1][1] = null;
        return result;
...
function delay_it_memo(exp, env) {
    let forced = false;
    let value;
    return {
        force: function() {
...
function apply(fun, args, env) {
    if (is_primitive_function(fun)) {
        ...
        const params_types = function_parameter_types(fun); // need to implement
        ...
        for (let i = 0; i < params.length; i++) {
            const arg_type = params_types[i];
            if (arg_type === "strict") {
                assign_symbol_value(params[i], actual_value(args[i], env), function_env);
            } else if (arg_type === "lazy") {
                assign_symbol_value(params[i], delay_it(args[i], env), function_env);
            } else if (arg_type === "lazy_memo") {
                assign_symbol_value(params[i], delay_it_memo(args[i], env), function_env);
            } else {
...
function driver_loop(env) {
    ...
    if (input === null) {
        ...
        const output = actual_value(program, program_env); // evaluate -> actual_value for lazy eval
...
function user_print(string, value) {
    function prepare(object) {
        return is_lazy_pair(object) ? print_lazy_list(object, 7) : // print 7 elements of lazy list
